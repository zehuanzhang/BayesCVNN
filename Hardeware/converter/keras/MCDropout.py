...
class ComplexBayesianDropout(BayesianDropout):
    r"""
    Applies Bayesian Dropout to complex inputs based on the specified parts (real or imaginary).
    """

    def __init__(self, drop_rate=0.5, real_imag=3, seed=None, **kwargs):
        super(ComplexBayesianDropout, self).__init__(drop_rate=drop_rate, seed=seed, **kwargs)

        # Define the setting for applying dropout to real, imag, or both
        self.setting = [(False, False), (False, True), (True, False), (True, True)]
        if real_imag not in range(4):
            raise ValueError("real_imag must be between 0 and 3")
        self.real, self.imag = self.setting[real_imag]
    ...
    def call(self, inputs):
        ...
        # Apply dropout based on the configuration
        if self.real:
            real_input = super(ComplexBayesianDropout, self).call(real_input)
        if self.imag:
            imag_input = super(ComplexBayesianDropout, self).call(imag_input)

        # Combine the real and imaginary components back into a complex output
        return tf.complex(real_input, imag_input)

    def get_config(self):
        config = {
            'real_imag': self.setting.index((self.real, self.imag))
        }
        base_config = super(ComplexBayesianDropout, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))