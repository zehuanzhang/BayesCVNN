...
class ComplexNet_LeNet_small(nn.Module):
    def __init__(self, init_channels, output_size, layer_setting, dropout_rate=0.25):
        super(ComplexNet_LeNet_small, self).__init__()
        self.init_channels = init_channels
        self.output_size = output_size
        self.layers = nn.ModuleList([
            ComplexConv2d(in_channels=self.init_channels, out_channels=3, kernel_size=5, padding=2, bias=False),
            ComplexMaxPool2d(kernel_size=2, stride=2),
            Complex_BernoulliDropout(dropout_rate, real_imag=layer_setting[0]),

            Flatten(),
            ComplexLinear(in_features=16 * 7 * 7, out_features=self.output_size, bias=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--training', default=False, help='training')
    ...
    layer_setting = (3,)
    if args.training:
        model = ComplexNet_LeNet_small(init_channels=1, output_size=10, layer_setting=layer_setting).to(device)
        ...
        torch.save(model, f'./trained_model/BayesNN_ComplexLeNet_small/complex_small_{layer_setting}.pth')

    if args.testing:
        # for layer_setting in layer_settings:
        model = torch.load(f'./trained_model/BayesNN_ComplexLeNet_small/complex_small_{layer_setting}.pth')
        print(model)
