...
from complexPyTorch.complexLayers import ComplexBatchNorm2d, ComplexConv2d, ComplexLinear, ComplexReLU, ComplexMaxPool2d, ComplexAvgPool2d
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride, args, dropout=False):
        ...
        self.stem.append(ComplexConv2d(
            in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False))
        self.stem.append(ComplexBatchNorm2d(planes))
        self.stem.append(ComplexReLU())
        if dropout:
            self.stem.append(BernoulliDropout(self.args.p))
        self.stem.append(ComplexConv2d(planes, planes, kernel_size=3,
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut.append(ComplexConv2d(in_planes, self.expansion*planes,
                          kernel_size=1, stride=stride, bias=False))
            self.shortcut.append(ComplexBatchNorm2d(self.expansion*planes))
            if dropout:
                self.shortcut.append(BernoulliDropout(self.args.p))

        self.add = Add()
        self.end = ComplexReLU()
...
class ResNet(nn.Module):
    def __init__(self, input_size, output_size, args, dropout=[False, False, False, False]):
        ...
        self.layers.append(ComplexConv2d(self.init_channels, self.in_planes, kernel_size=3, stride=1, padding=1, bias=False))
        self.layers.append(ComplexBatchNorm2d(self.in_planes))
        ...
        self.layers.append(ComplexAvgPool2d(4))
        self.layers.append(Flatten())
        self.layers.append(
            ComplexLinear(192*BasicBlock.expansion, 192*BasicBlock.expansion, bias=False))
        self.layers.append(ComplexReLU())
...
class ResNet_P_complex(ResNet):
    def __init__(self, input_size, output_size, args):
        super(ResNet_P_complex, self).__init__(input_size, output_size, args, [False, False, False, False])
        self.layers.append(
            ComplexLinear(192*BasicBlock.expansion, output_size, bias=False))
...
class ResNet_LL_complex(ResNet):
    def __init__(self, input_size, output_size, args):
        super(ResNet_LL_complex, self).__init__(input_size,
                                        output_size, args, [False, False, False, False])
        print('args.real_imag:', args.real_imag)
        self.layers.append(Complex_BernoulliDropout(self.args.p, real_imag=args.real_imag))
        self.layers.append(
            ComplexLinear(192*BasicBlock.expansion, output_size, bias=False))
