...
class Trainer():
  ...
  def _step(self, input, target, optimizer, epoch, n_batches, n_points, train_timer):
    ...
    if next(self.model.parameters()).is_cuda:
      # input = input.cuda()
      input = input.cuda().type(torch.complex64) #input torch.complex64 (CUDAComplexFloatType)
      target = target.cuda()
    if optimizer is not None:
      optimizer.zero_grad()

    # output = self.model(input)
    output = self.model(input)  # input torch.complex64 (CUDAComplexFloatType)  torch.Size([256, 10])
