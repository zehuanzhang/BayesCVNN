...
from Src.ComplexValuedAutoencoder_Class_Torch import ComplexBatchNorm1d, ComplexBatchNorm2d, ComplexConv1d, ComplexConv2d, ComplexLinear, ComplexConvTranspose1d, ComplexConvTranspose2d, ComplexReLU, ComplexLeakyReLU, ComplexSigmoid, Complexavg_pool1d, Complexavg_pool2d, ComplexUpsample, ComplexConv2d_RI


#######################################################################################


class Complex_DoubleConv_RI(nn.Module):
    """(convolution => [BN] => ReLU) * 2"""

    def __init__(self, in_channels, out_channels_r, out_channels_i, mid_channels=None, kernel_size=3, activation="Relu"):
        super().__init__()
        if not mid_channels:
            mid_channels_r = out_channels_r
            mid_channels_i = out_channels_i
        if activation == "Relu":
            self.double_conv = nn.Sequential(
                ComplexConv2d_RI(in_channels=in_channels, out_channels_r=mid_channels_r, out_channels_i=mid_channels_i, kernel_size=kernel_size, stride=1, padding=1, dilation=1, groups=1, bias=True),
                ComplexBatchNorm2d(num_features=max(mid_channels_r, mid_channels_i), eps=1e-5, momentum=0.1, affine=True, track_running_stats=True),
                ComplexReLU(),
                ComplexConv2d_RI(in_channels=max(mid_channels_r, mid_channels_i), out_channels_r=out_channels_r, out_channels_i=out_channels_i, kernel_size=kernel_size, stride=1, padding=1, dilation=1, groups=1, bias=True),
                ComplexBatchNorm2d(num_features=max(out_channels_r, out_channels_i), eps=1e-5, momentum=0.1, affine=True, track_running_stats=True),
                ComplexReLU()
            )
...
class Complex_Down_DoubleConv2d_RI(nn.Module):
    """Downscaling with avgpool then double conv"""

    def __init__(self, in_channels, out_channels_r, out_channels_i, Conv2d_kernel_size=3, Pooling_kernel_size=2):
        super().__init__()
        self.pool_conv = nn.Sequential(
            ...
            Complex_DoubleConv_RI(in_channels, out_channels_r, out_channels_i, mid_channels=None, kernel_size=Conv2d_kernel_size)
        )
...
class Complex_Linear(nn.Module):
    ...
    def forward(self, x):
        # print(f'Net parts torch: {x.shape}')
        return self.lin(x)
