...
#
#
# Function to pad channels
def pad_tensor_to_channels(tensor, target_channels):
    batchsize, C, H, W = tensor.size()
    if C == target_channels:
        return tensor
    else:
        # Calculate padding needed
        padding_needed = target_channels - C
        # Create a new tensor with the target number of channels
        padded_tensor = torch.zeros((batchsize, target_channels, H, W), dtype=tensor.dtype).to('cuda')#.  to('cpu')
        # Copy the original tensor's data to the padded tensor
        padded_tensor[:, :C, :, :] = tensor
        return padded_tensor

def apply_complex_RI(fr, fi, input, dtype=torch.complex64):

    # fr_ir = fr(input.real) #(128,32,32,32)
    # fi_ii = fi(input.imag)
    # fr_ii = fr(input.imag)
    # fi_ir = fi(input.real)

    #weight shape: (output_channel, input_channel, kernel_size, kernel_size)
    input_channel = input.shape[1]
    fr_channel = fr.weight.shape[1]
    fi_channel = fi.weight.shape[1]

    if input_channel == fr_channel:
        fr_ir = fr(input.real)
        fr_ii = fr(input.imag)
    elif input_channel > fr_channel:
        fr_ir = fr(input.real[:fr_channel])
        fr_ii = fr(input.imag[:fr_channel])
    else:
        raise ValueError("input_channel < fr_channel")

    if input_channel == fi_channel:
        fi_ii = fi(input.imag)
        fi_ir = fi(input.real)
    elif input_channel > fi_channel:
        fi_ii = fi(input.imag[:fr_channel])
        fi_ir = fi(input.real[:fr_channel])
    else:
        raise ValueError("input_channel < fi_channel")

    target_channels = max(fr_ir.size(1), fi_ii.size(1))

    fr_ir_pad = pad_tensor_to_channels(fr_ir, target_channels) # torch.Size([128, 64, 32, 32])
    fi_ii_pad = pad_tensor_to_channels(fi_ii, target_channels) # torch.Size([128, 32, 32, 32])
    target_channels = max(fr_ii.size(1), fi_ir.size(1))
    fr_ii_pad = pad_tensor_to_channels(fr_ii, target_channels)
    fi_ir_pad = pad_tensor_to_channels(fi_ir, target_channels)
    # print(fr_ir_pad.device, fi_ii_pad.device)
    return (fr_ir_pad-fi_ii_pad).type(dtype) \
        + 1j*(fr_ii_pad+fi_ir_pad).type(dtype)
#
#
#
...
class ComplexConv2d_RI(Module):
    def __init__(
        ...
        super(ComplexConv2d_RI, self).__init__()
        self.conv_r = Conv2d(
            in_channels,
            out_channels_r,
            kernel_size,
    ...
    def forward(self, inp):
        # print('inp.shape:', inp.shape, inp.device)
        return apply_complex_RI(self.conv_r, self.conv_i, inp)
#
#
#
...
class Complex2foldloss(Module):
    ...
    def forward(self, reconstroction_predicted, reconstroction_target, classification_predicted, classification_target):
        ...
        classification_loss = classification_loss_madule(classification_predicted, classification_target)
        # print('loss classification_predicted:', classification_predicted)
        # print('loss classification_target:', classification_target)
...
class Complex2foldloss_Coh(Module):
    ...
    def forward(self, reconstroction_predicted, reconstroction_target, classification_predicted, classification_target):
        ...
        # reconstroction_predicted (10,2,100,100)    reconstroction_target (10,2,100,100)
